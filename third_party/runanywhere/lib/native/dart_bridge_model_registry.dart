  ...
  Future<bool> saveModel(ModelInfo model) async {
    ...
    try {
      ...
      try {
        ...
        // Do not populate fields beyond localPath here. The Android 0.16.0
        // native library appears to expose a rac_model_info_t layout that does
        // not match the extended Dart struct below. Writing later fields can
        // corrupt native string pointers, which crashes in rac_strdup() inside
        // rac_model_registry_save(). rac_model_info_alloc() zeroes the struct,
        // so leaving the remaining metadata empty is safe for registration,
        // download discovery, and direct model loading.
